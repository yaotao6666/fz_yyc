...
- 小程序场景如需 `image` 直接加载 SVG，请确认运行环境对 SVG 的支持；若受限可导出 PNG 后替换。
- 推荐统一以 `1024x1024` 作为源文件，后续按需等比缩放。

当前项目默认应用：

- 登录页品牌 Logo：`BrandAsset.APP_LOGO`（`icon-circle-aggregated-portal.svg`）
- 商家/店铺缺省头像：`BrandAsset.DEFAULT_MERCHANT_LOGO`（`icon-circle-store-data-butler.svg`）
