...
- 目标：在 PC 端 web-admin 的商品编辑页增加手动修改商品销量的能力，仅限服务商访问。
- 已确认决策：
  - 修改入口：在商品编辑页（MerchantProductEditView）增加“销量”输入框。
  - 权限范围：仅服务商可以通过 web-admin 修改任意商家的商品销量，商家端保持只读。
...
- web-admin 目前使用服务商 token，权限通过 `SpAuth` 中间件校验。
- 商品相关接口挂在 `merchantGroup` 下，当前只允许商家访问，服务商无法直接调用。
- 上一轮已实现商品域接口兼容服务商 token（通过 `resolveTargetMerchantID` 解析目标商家并校验归属），但更新接口本身没有 sales 字段。
...
- 改动点：
  ...
  - 增加 `v-if="isServiceProvider"`，仅服务商可见。
  - 字段校验：必须为非负整数。
- 说明：
...
- 补充内容：
  - 说明 web-admin 支持服务商手动修改商品销量，仅限 PC 端。
...
- 假设：服务商修改销量是运营需求，不需要记录修改历史（如需可后续扩展）。

## Verification Steps

### 后端接口验证
- 服务商 token 调用 `PUT /api/v1/merchant/products/:id` 时，带上 `sales` 字段可成功更新。
- 服务商 token 调用时不带 `sales` 字段时，接口行为与之前保持一致（不更新销量）。
- 商家 token 调用接口时，可以正常更新商品信息（不包含 sales）。
- 服务商 token 指向非本服务商名下商家时被拒绝。

### 前端页面验证
- 服务商登录 web-admin，进入商品编辑页，能看到“销量”输入框。
...
2. **后端：确认 resolveTargetMerchantID 已支持 sp 身份**
   ...
   - 确认服务商 token 可以通过 `merchant_id` 指定目标商家并校验归属。
...
4. **前端：在商品编辑页增加销量输入框**
   ...
   - 添加 `v-if="isServiceProvider"` 限制仅服务商可见。
   - 设置默认值为 0，最小值为 0。

5. **文档同步**
   - 在 `PRD.md` 中增加 web-admin 商品销量修改能力说明。
   - 在 `docs/prd/PRD-功能说明.md` 中补充服务商端商品管理能力细节。
