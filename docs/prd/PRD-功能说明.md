...
- `web-admin/src/views/merchant/MerchantProductEditView.vue`：服务商商品新增与编辑
...
- `web-admin/src/views/merchant/MerchantDetailView.vue` 新增 `基础资料 / 商品管理 / 分类管理` Tab，服务商可直接代管指定商家的商品与分类
...
- 系统通过全局配置 `WECHAT_PAY_APP_MODE` 切换当前部署版本的小程序支付身份：
  - `sp_app`：服务商主体小程序，支付使用 `sp_appid + sp_openid`
  - `sub_app`：非服务商主体小程序，支付仍走服务商体系，请求中继续保留 `sp_appid`，并使用 `sub_appid + sub_openid`
- `sp_app` 与 `sub_app` 仅切换小程序登录与下单身份，支付回调、退款、分账仍沿用服务商支付链路。
