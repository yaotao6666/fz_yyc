...
- 商户公告为首页单行店铺通知，由 PC 后台「系统设置·商家资料」维护，落库于 `merchants.announcement` 文本字段，通过 `PUT /api/v1/merchant/profile` 更新。

- C 端商城首页公告展示读取 `merchants.announcement` 字段。

- 不再维护独立的公告表与公告 CRUD 接口（与健康宣教互为独立的文章库模块，互不重复）。
...
- 系统公告：`PUT /api/v1/merchant/profile`（商家资料 `announcement` 字段）
...
- `GET /api/v1/merchant/orders` 需支持 `status`、`category`（1=实物订单 order\_type 1-2 / 2=服务订单 order\_type 3-6，不传=全部）、`order_type`、`biz_status`、`start_date`、`end_date`、`keyword`、`page`、`page_size` 筛选，返回当前商户名下订单。
...
- 商户公告由 PC 后台通过 `PUT /api/v1/merchant/profile` 的 `announcement` 字段维护（首页单行店铺通知）。
...
- 待接订单列表：`GET /api/v1/service-staff/orders/pending`
  ...
  - 支持 `page`、`page_size`、`sort`（传 `scheduled_at` 按预约时间升序，无预约时间排后；默认按 `paid_at` 倒序）

  - 区域过滤保留：按服务人员 `service_region` 与订单 `delivery_district` 匹配，区域为空视为不限

  - 每条返回 `customer` 增强信息（见下「工单「人」的信息增强」）

- 接单：`POST /api/v1/service-staff/orders/:id/accept`

  - 原子抢占：仅 `status=2 且 biz_status=1 且 assigned_staff_id IS NULL` 时成功，写入 `assigned_staff_id`、`assigned_at`、`biz_status=2`

  - 接单成功后异步 best-effort 通过微信订阅消息通知下单用户（内容含服务人员姓名/电话/预约时间），模板 ID 走环境变量 `WECHAT_STAFF_ACCEPT_TEMPLATE_ID`（未配置则跳过该通道），结果写入 `order_notify_logs` 表并按 `order_id + channel="wechat"` 幂等去重（不阻塞接单主流程）

- 放弃工单：`POST /api/v1/service-staff/orders/:id/give-up`

  - 仅本人已接单且待出发（`assigned_staff_id=本人 且 biz_status=2`）的工单可原子退回待接单池：`biz_status→1`、`assigned_staff_id→NULL`、`actual_started_at→NULL`

  - 服务中（`biz_status=3`）或非本人指派返回错误，工单不可放弃

- 已接订单列表：`GET /api/v1/service-staff/orders/accepted`

  - 支持 `page`、`page_size`、`biz_status`（可选筛选 2 已接单 / 3 服务中 / 5 已完成）；每条返回 `customer`

- 待办列表：`GET /api/v1/service-staff/todo`

  - 返回待出发 + 服务中（`biz_status ∈ {2,3}`）工单及今日统计（`stats.today_assigned` / `stats.today_completed`）；每条返回 `customer`

- 订单详情：`GET /api/v1/service-staff/orders/:id`

  - 仅本人已接或待接订单可见（他人返回 403）；返回 `customer`
...
- 放弃工单为原子操作，仅本人待出发（`biz_status=2`）工单可退回待接单池，避免并发冲突与越权放弃。

- **工单「人」的信息增强**：`OrderDetail` / `PendingOrders` / `AcceptedOrders` / `GetTodoList` 统一回填 `customer` 对象，字段口径如下：

  - `customer.user`：下单人画像 `nickname`（昵称）/ `avatar`（头像）/ `phone` / `total_orders`（累计订单）/ `total_spent`（累计消费）

  - `customer.record`：服务对象（`orders.record_id` 关联健康档案）`id` / `real_name`（姓名）/ `gender`（1男2女）/ `birth_date`（出生日期，前端据此算年龄）/ `relation`（1本人2父母3其他亲属）/ `assessment_level`（评估等级）/ `phone` / `address`；健康警示字段 `allergy_history`（过敏史）/ `chronic_tags`（慢病标签）/ `medication_list`（长期用药）/ `emergency_contact` / `emergency_phone`（紧急联系人）——敏感信息前端折叠/脱敏展示

  - 数据权限：服务对象信息仅取本单 `record_id` 指向的档案（服务人员因接本单获得查看权），不返回其他档案，且不包含身份证等敏感字段

- 接单通知：环境变量 `WECHAT_STAFF_ACCEPT_TEMPLATE_ID` 未配置或下单用户无 `openid` 时跳过通知，仅写 `order_notify_logs` 留痕；通知失败不影响接单主流程。

- 新增 `order_notify_logs` 表（表前缀 `order_`，非 `service_`）承载接单通知留痕。
...
| `POST /residents/:user_id/fitting-recommendations` | 为客户生成适配建议，请求 `{assessment_id?, symptom_desc, fitting_result, recommended_products:[{product_id, reason}]}`，推荐商品逐一校验并快照 name/sale\_type，落库 `status=0`（数据权限校验） |
...
| `PUT /fitting-recommendations/:id`    | `fitting:update` | 编辑适配建议（结论/推荐商品/状态/order\_id，仅更新传入字段，推荐商品重新校验并快照） |
...
- **删除保护**：存在照护记录（care\_visits）的照护计划不可删除，保留历史溯源。
