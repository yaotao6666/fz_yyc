...
- 服务商端：
  ...
  - 服务商端 `web-admin` 在商家详情中新增商品管理与分类管理 Tab，可直接代管指定商家的商品、分类、规格与上下架操作，服务商可手动修改商品销量。
- 文档与测试：
...
**说明**：服务商通过独立的 Web/PC 后台登录，统一管理商家、支付配置、分账历史与统计分析能力，与商家端和 C 端小程序隔离。

**当前模式**：

- 系统采用单服务商模式，不存在平台角色。
- 当前服务商能力已迁移到 `web-admin/`，不再提供服务商小程序入口。
- 唯一服务商承担传统平台职责。
...
为方便服务商日常运营管理，当前提供独立的 `web-admin/` 服务商 Web 后台，通过浏览器访问，与商家管理小程序完全独立。

**访问路径**：

```
浏览器打开服务商后台 → 进入登录页 → 输入账号密码登录 → 进入服务商管理端
```
...
| 系统公告接口 | 小程序页面已下线，后端接口保留供后续 PC 端复用 | P1  |
...
- 后端继续保留 `/api/v1/sp/announcements*` 接口，供后续 PC 端复用
- 商家端可在首页查看最新公告
...
1. **服务商端**：通过 `web-admin/` 登录后进入服务商管理端，用于创建商家、维护支付配置、查看统计与维护服务商设置。当前系统采用单服务商模式，不存在独立平台后台。
2. **商家端**：商家员工登录后进入经营后台，用于商品、订单、设置与员工能力联调。
3. **C 端店铺端**：用户浏览店铺并下单的链路测试，由于开发阶段无法总是通过扫码直达指定商家店铺，因此提供测试入口页与编译模式两种方式。

开发测试时，需明确区分以下入口：

- `web-admin/`：服务商 Web/PC 后台项目入口
...
| 商家管理 | 商家列表、商家详情、支付配置 | `/merchants` / `/merchants/:id` / `/merchants/:id/edit` |
| 订单管理 | 按商家筛选查看订单与详情   | `/orders` / `/orders/:id`                             |
| 分账历史 | 查看抽佣处理记录       | `/profit-sharing`                                     |
| 数据分析 | 商家统计           | `/analytics`                                          |
| 设置   | 修改密码、维护服务商资料   | `/settings`                                           |
