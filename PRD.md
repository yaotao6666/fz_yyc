...
- `pages/store/product`：商品详情（含科普资讯 product\_type=5 正文展示）
...
- `sort`：排序方式，可选 `scheduled_at` 按预约时间（`scheduled_at`）升序；不传则按支付时间（`paid_at`）降序
  ...
  "data": [
    {
      ...
      "scheduled_at": "2024-01-01T10:30:00Z",
...
- `customer.user`：下单人信息，含昵称、头像、累计订单数与累计消费金额。

- `customer.record`：服务对象健康档案快照（姓名/性别/生日/与下单人关系/评估等级/电话/地址），并带健康警示：过敏史、慢病标签、用药清单、紧急联系人。健康警示类字段（过敏史/慢病/用药）建议折叠脱敏展示，不含身份证等敏感字段；`record` 为空时不展示服务对象卡片。
...
- 接单为原子操作（`biz_status=1` 且 `assigned_staff_id IS NULL` 才可接单），避免多服务人员重复接单。

- 接单成功后订单写入 `assigned_staff_id`、`assigned_at`（接单/指派时间），`biz_status` 置为 2（已接单-待出发），并清空 `actual_started_at`。

- 接单成功后，后端异步「尽力而为」（best-effort，失败吞错、不阻塞响应）地向下单用户发送微信订阅消息，携带服务人员姓名、联系电话与预约时间；该逻辑已实现，若 `WECHAT_STAFF_ACCEPT_TEMPLATE_ID` 未配置则跳过发送。发送记录（含成功/失败）写入 `order_notify_logs`，按 `order_id + channel=wechat` 幂等去重，同一订单不会重复发送。
...
```
POST /api/v1/service-staff/orders/{id}/give-up
Authorization: Bearer {staff_token}
```

**说明：**

- 原子操作：仅本人已接单（`assigned_staff_id=本人`）且为待出发（`biz_status=2`）状态时可将工单退回待接单池。

- 成功后订单更新：`biz_status`→1（退回待接单）、`assigned_staff_id`→NULL、`actual_started_at`→NULL。

- 非本人或非待出发状态（如已签到开始服务）无法放弃：`biz_status != 2` 时返回 400（仅待出发工单可放弃，或工单已变更）。
...
- `biz_status`：可选筛选，2=已接单(待出发)、3=服务中、5=已完成；不传返回本人全部工单
...
- 列表同样回填 `customer`（下单人 + 服务对象健康档案快照），结构同 3.2.3 待接订单列表。

#### 3.2.7 待办列表

```
GET /api/v1/service-staff/todo
Authorization: Bearer {staff_token}
```
    ...
    "list": [
      {
        ...
        "contact_phone": "13800138000",
        "customer": {
          "user": { "nickname": "用户昵称", "avatar": "头像URL" },
          "record": { "id": 3, "real_name": "张三", "gender": 1, "assessment_level": "轻度失能" }
        }
      }
    ],
    "stats": {
...
- 返回当前服务人员「待出发（`biz_status=2`）+ 服务中/已签到（`biz_status=3`）」的全部待办工单，供小程序工作台展示。

- 列表项带 `customer`（下单人 + 服务对象快照），结构同 3.2.3。

- `stats.today_assigned`：今日已接单数（按 `actual_started_at` 落在今日统计）。

- `stats.today_completed`：今日已完成数（`biz_status=5` 且 `actual_ended_at` 落在今日）。
...
- 详情同样回填 `customer`（下单人 + 服务对象健康档案快照），结构同 3.2.3。

- 订单只能查看本人已接或待接的工单（`assigned_staff_id` 非本人且非空时返回 403 无权查看）。
...
- 待接订单列表、已接订单列表与待办列表均需保证空数组兜底（待办返回 `list` 空数组 + `stats`）。

- 接单与放弃工单均为原子操作，避免多服务人员重复接单/竞争。

- 支付成功后，后端向服务人员小程序推送待接订单提醒。

- **签名勘误**：接单响应中的接单服务人员字段为 `assigned_staff_id`（非 `accepted_by_staff_id`），订单接单状态字段为 `assigned_at`（原章节旧文案 `accepted_by_staff_id`/`accepted_at` 为占位误区，已按真实代码修正）。工单业务状态统一以 `biz_status` 表达（1=待接单、2=已接单-待出发、3=服务中、5=已完成）。
...
- `activities`（活动信息数组，`type` 为 `full_reduction` 满减规则摘要）已随平台的 `activities` 运营活动表一并下线，后端不再返回该字段；促销统一走优惠券体系（`coupon_templates` / `user_coupons`）。
...
- `GET /health-records`（`health:view`）、`GET /health-records/:id`（`health:view`）、`PUT /health-records/:id`（`health:update`）、`GET /health-records/:id/assessments`（`health:assessment`，按档案查看评估记录，健康档案列表「评估记录」按钮独立拉取）
...
- **服务人员端**（前缀 `/api/v1/service-staff`，需服务人员登录）：
  ...
  - `POST /residents/:user_id/fitting-recommendations`：生成适配建议，请求 `{assessment_id?, symptom_desc, fitting_result, recommended_products:[{product_id, reason}]}`，推荐商品校验并快照 name/sale\_type，落库 `status=0`（数据权限校验）
...
- **管理端**（前缀 `/api/v1/merchant` + RBAC）：
  ...
  - `PUT /fitting-recommendations/:id`（`fitting:update`，更新结论/推荐商品/状态/order\_id）
...
- **服务人员端**（前缀 `/api/v1/service-staff`，需服务人员登录）：
  ...
  - `POST /care-visits`：录入上门照护记录，请求 `{plan_id?, order_id?, visit_at, nursing_items:[{name,done,remark}], vitals, photos, remark, follow_up_advice}`（plan\_id/order\_id 至少其一且均属当前服务人员，user\_id 取 plan/order 对应且一致）
...
| 值 | 状态标识       | 描述    | 说明                       |
| - | ---------- | ----- | ------------------------ |
| 0 | incomplete | 未完成配置 | 未回填 sub\_mch\_id         |
...
- ✅ **RBAC 菜单**：新增顶级菜单「健康服务」`/health`（id=8），下含健康档案（`health:view` / `health:update` / `health:assessment` 按档案查看评估记录，迁移脚本 `026_health_record_assessments.sql`）、评估量表（`assessment:view` / `assessment:create/update/delete`）、评估记录（`assessment:view` / `assessment:create`）；迁移脚本自动将全部菜单绑定角色 1
