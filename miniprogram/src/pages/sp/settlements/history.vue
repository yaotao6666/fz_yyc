  ...
  <view class="history-container">
    <view class="filter-card">
      ...
      <view class="filter-row">
        <picker mode="selector" :range="merchantOptions" range-key="name" :value="merchantIndex" @change="onMerchantChange">
          <view class="picker-field">{{ merchantOptions[merchantIndex]?.name || '全部商家' }}</view>
        </picker>
    ...
    <scroll-view class="history-scroll" scroll-y @scrolltolower="loadMore">
      <view v-for="record in records" :key="record.id" class="record-card">
        ...
        <view v-if="record.error_message" class="error-text">失败原因：{{ record.error_message }}</view>
      </view>
      ...
      <view v-else-if="records.length === 0" class="empty-state">
        ...
        <text class="empty-desc">支付完成并命中分账比例后，会在这里沉淀服务商抽佣历史。</text>
      </view>
...
import { getMerchantList, getSpProfitSharingRecords } from '@api'
import type { MerchantListItem, ProfitSharingRecord } from '@types'

const merchantOptions = ref<Array<{ id?: number; name: string }>>([{ name: '全部商家' }])
...
onLoad((options) => {
  const merchantId = Number(options?.merchant_id || 0)
...
async function loadMerchantOptions() {
  try {
    ...
    merchantOptions.value = [{ name: '全部商家' }].concat(
      response.list.map((item: MerchantListItem) => ({
        id: item.id,
        name: item.name
      }))
    )
    if (filters.merchant_id) {
      const matchedIndex = merchantOptions.value.findIndex(item => item.id === filters.merchant_id)
      merchantIndex.value = matchedIndex > 0 ? matchedIndex : 0
    }
  } catch (requestError) {
    console.error('加载商家选项失败:', requestError)
  }
}
...
function onMerchantChange(event: any) {
  merchantIndex.value = Number(event.detail.value || 0)
  filters.merchant_id = merchantOptions.value[merchantIndex.value]?.id
  reloadRecords()
}
