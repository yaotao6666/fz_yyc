...
func main() {
	...
	checks := []countCheck{
		...
		{label: "商家进件申请数量", query: "SELECT COUNT(*) FROM merchant_applications", minCount: 10},
		...
		{label: "营业执照数量", query: "SELECT COUNT(*) FROM merchant_licenses", minCount: 10},
		...
		{label: "邀请记录数量", query: "SELECT COUNT(*) FROM invite_records", minCount: 9},
		{label: "平台活动数量", query: "SELECT COUNT(*) FROM activities", minCount: 4},
		{label: "邀请奖励数量", query: "SELECT COUNT(*) FROM invite_rewards", minCount: 3},
		{label: "系统公告数量", query: "SELECT COUNT(*) FROM announcements", minCount: 5},
		{label: "审核记录数量", query: "SELECT COUNT(*) FROM merchant_audit_records", minCount: 10},
	...
	relationshipChecks := []countCheck{
		...
		{
			label:    "每个商家都有营业执照",
			query:    "SELECT COUNT(*) FROM merchants m LEFT JOIN merchant_licenses l ON l.merchant_id = m.id WHERE l.id IS NULL",
			minCount: 0,
		},
		{
			label:    "至少存在带商品的商家",
		...
		{
			label:    "至少存在已完成邀请记录",
			query:    "SELECT COUNT(*) FROM invite_records WHERE status = 1",
			minCount: 6,
		},
	}
