...
**产品定位**：为单商户打造的适老化项目经营平台，客户端下单、服务人员接单、PC 后台管理三端协同。

**核心原则**：客户端小程序围绕**用户下单**为中心；服务人员小程序围绕**接单**为中心；PC 后台围绕**经营管理**为中心。
...
└─────────────────────────────────────────────────────────────────────────────┘
                    ...
                    ▼                   ▼                   ▼
          ┌─────────────────┐ ┌─────────────────┐ ┌─────────────────┐
          │  客户端小程序     │ │ 服务人员接单小程序  │ │  商户PC后台       │
          │  (商城入口)      │ │  (独立小程序)     │ │  (Web入口)       │
          │                 │ │                 │ │                 │
          │ • 扫码进入商城首页 │ │ • 注册/登录      │ │ • 商户登录       │
          │ • 首页/分类/购物车 │ │ • 查看待接订单   │ │ • 商品/分类管理   │
          │   /我的 tabBar   │ │ • 接单           │ │ • 订单/核销管理   │
          │ • 商品浏览下单   │ │ • 已接订单与历史  │ │ • 满减/打印  │
          │ • 支付购买       │ │                 │ │ • 服务人员管理    │
          │ • 我的订单       │ │                 │ │ • 数据分析        │
          └─────────────────┘ └─────────────────┘ └─────────────────┘
```
...
**客户端小程序设计风格**
为一个面向 C 端用户的适老化项目商城下单平台设计图标，产品名称为"财旭商贸"。
...
**说明**：客户端小程序是面向 C 端用户的下单入口，采用单商户商城式布局，底部 tabBar 导航（首页、分类、购物车、我的）。用户通过扫描商户二维码进入商城首页，进行商品浏览、下单购买等操作。小程序不包含任何商家管理能力。

**核心功能**：

| 模块   | 功能描述                              |
| ---- | --------------------------------- |
| 首页   | 轮播图、商品活动、服务分类图标宫格、热销推荐            |
| 分类   | 全部分类列表、分类下商品浏览                    |
| 购物车  | 商品数量修改、批量结算                       |
...
```
商户二维码 → 扫码 → 进入商城首页（tabBar）→ 浏览轮播/活动/分类图标/商品 → 加购 → 切换 tabBar 到购物车 → 结算下单 → 支付
```
...
- 用户登录后可在「我的」tab 查看历史订单记录
- 支持订单状态筛选（待支付/已支付/已完成/退款中）

**首页优化功能**：

- **操作指引弹窗**：用户首次进入商城首页时，自动显示操作指引弹窗，包含商城购物流程说明；关闭后不再自动弹出
- **加购弹窗**：在首页或分类页点击商品「+」时弹出规格与数量选择，确认后加入购物车并累计金额
- **tabBar 购物车入口**：底部 tabBar 购物车 tab 展示购物车商品汇总与去结算动作
- **自动授权登录**：用户进入商城首页时自动完成微信授权登录，获取 openid 作为唯一身份标识，无需手动登录
  - openid 同时用于访问/行为埋点归因；登录需做并发保护，避免进店时重复触发多次登录请求
...
3. 调用后端微信登录接口，传入授权码
4. 后端根据授权码获取 openid
5. 查询或创建用户记录（openid 作为唯一标识）
6. 返回 `token + user(openid)`，前端写入 `user_token` 与 `openid` 并用于后续下单与订单查询
7. 前端保存 token，后续请求携带 token
8. 用户二次访问时，直接通过 openid 识别用户，无需重新授权

**openid 管理**：

- openid 是用户在微信生态下的唯一标识
- 不同小程序的 openid 不同，但同一小程序的 openid 不变
- 用户二次访问时，直接通过 openid 识别用户，无需重新授权

**隐私保护**：

- 仅获取用户 openid，不获取手机号等敏感信息
- 用户昵称默认显示为"微信用户"
- 后续可扩展用户主动完善个人信息

### 1.5 商户 PC 后台（web-admin）

**说明**：商户 PC 后台是单商户的统一管理后台，通过浏览器访问，承接商品、分类、订单、核销、满减、打印机、公告、服务人员、数据分析与设置等全部经营管理能力。
...
| 模块     | 功能描述                          |
| ------ | ----------------------------- |
| 数据看板   | 今日订单、交易额、客单价、订单趋势             |
| 商品管理   | 商品新增/编辑/上下架/删除、库存管理、规格管理       |
| 分类管理   | 商品分类创建/编辑/删除/排序               |
| 订单管理   | 订单列表、订单详情、退款处理                |
| 订单核销   | 扫码核销、订单详情核销                   |
| 满减营销   | 配置多档满减规则                      |
| 云打印    | 打印机配置/管理、自动打印、测试打印            |
| 公告管理   | 发布/编辑/停用系统公告                  |
...
**说明**：服务人员接单小程序是面向服务人员的独立小程序，服务人员通过注册申请（经 PC 后台审核）或由 PC 后台直接添加后登录，查看待接订单并执行接单。服务人员不执行核销。

**核心功能**：

| 模块     | 功能描述                          |
| ------ | ----------------------------- |
| 注册申请   | 服务人员在小程序端提交注册申请，待 PC 后台审核    |
| 登录     | 账号密码或微信快捷登录                  |
| 待接订单   | 查看待接订单列表                      |
| 接单     | 对待接订单执行接单，订单记录接单人             |
| 已接订单   | 查看已接订单与历史                     |
...
- 服务人员为独立身份，与商户管理员账号（`merchant_staffs`）分离
- 注册来源：PC 后台直接添加（`admin`）或小程序端注册申请（`self`，需审核）
...
- 商品列表、商品详情、C 端商城商品接口在返回图片时，由后端按当前七牛策略动态生成可直接访问的地址。
...
- **资料维护**：商户管理员在 PC 后台维护商户基础资料、Logo、门头图等经营信息
- **支付配置**：对已完成线下进件的商户回填 `sub_mch_id`
...
| ---- | --------------------------- |
| 资料维护 | 维护商户基础资料和图片资源               |
| 支付配置 | 回填已进件商户的 `sub_mch_id`       |

#### 1.8.1 商户 PC 后台说明

商户 PC 后台（`web-admin/`）通过浏览器访问，是单商户的统一管理后台。

**访问路径**：

```
浏览器打开商户后台 → 进入登录页 → 输入账号密码登录 → 进入商户管理端
```
...
```
商户管理员登录 PC 后台 → 维护商户资料 → 回填 sub_mch_id（线下进件完成后） → 配置校验通过 → 可正常拉起支付
```
...
| 状态值 | 说明    | 口径                         |
| --- | ----- | -------------------------- |
| 0   | 未完成配置 | 未回填 `sub_mch_id`  |
| 1   | 已完成配置 | 已回填 `sub_mch_id`，可正常拉起支付   |

#### 1.8.3 线下进件说明

进件方式为线下，不属于系统内流程。当前产品口径如下：

- 商户不在系统中提交进件资料
- 微信侧进件完成后，仅将结果体现为可回填的 `sub_mch_id`
- 商户管理员在 PC 后台回填 `sub_mch_id` 后，即可进入支付链路

**线下进件结果在系统中的表现：**

| 结果    | 系统表现                             |
| ----- | -------------------------------- |
| 已进件成功 | 商户管理员可回填 `sub_mch_id`，支付配置状态置为完成 |
| 未完成进件 | 不回填 `sub_mch_id`，下单时提示支付配置未完成     |
...
```
用户扫描商户二维码 → 进入商城首页（tabBar）→ 浏览轮播/活动/分类图标/商品 → 选择商品加入购物车 → 切换 tabBar 到购物车结算 → 确认订单信息 → 选择已开启的下单方式（配送 / 堂食 / 自提）→ 调用微信支付 → 支付成功回调 → 更新订单状态 → 服务人员接单 → 商户管理员核销 → 用户查看订单
```
...
| 功能     | 描述                          | 优先级 |
| ------ | --------------------------- | --- |
| 商户管理员登录 | 商户管理员账号密码登录 PC 后台          | P0  |
| 首页数据看板 | 今日订单、交易额、客单价、订单趋势          | P0  |
| 商品管理   | 商品新增/编辑/上下架/删除、库存管理、规格管理    | P0  |
| 分类管理   | 商品分类创建/编辑/删除/排序            | P0  |
| 订单管理   | 订单列表、订单详情、退款处理             | P0  |
| 订单核销   | 扫码核销、订单详情核销                | P0  |
| 满减营销   | 配置多档满减规则                    | P0  |
| 云打印    | 打印机配置/管理、自动打印、测试打印         | P1  |
| 公告管理   | 发布/编辑/停用系统公告               | P1  |
| 服务人员管理 | 添加/审核/启停服务人员账号             | P0  |
| 数据分析   | 销售统计、商品排行、库存预警、时段分析        | P1  |
| 商户设置   | 商户资料、支付配置、配送设置、营业状态        | P0  |
...
| 订单核销 | 商户管理员在 PC 后台核销订单    | P1  |
| 订单退款 | 商户管理员在 PC 后台处理退款    | P1  |
...
| ---- | --------------------- | --- |
| 微信登录 | 用户授权登录                | P0  |
| 扫码进入商城 | 扫描商户二维码进入商城首页（tabBar） | P0  |
| 商城首页 | 轮播图、商品活动、分类图标宫格、热销推荐  | P0  |
| 商品浏览 | 浏览商品列表                | P0  |
...
| 功能     | 描述                          | 优先级 |
| ------ | --------------------------- | --- |
| 注册申请   | 服务人员在小程序端提交注册申请，待 PC 后台审核  | P0  |
| 登录     | 账号密码或微信快捷登录                | P0  |
| 待接订单   | 查看待接订单列表                    | P0  |
| 接单     | 对待接订单执行接单，订单记录接单人          | P0  |
| 已接订单   | 查看已接订单与历史                   | P0  |
| 订单详情   | 查看订单商品、地址、金额等信息             | P0  |
| 接单统计   | 查看今日/历史接单数与金额               | P1  |

### 2.9 实时提醒模块

**PC 后台实时提醒**：

- 商户管理员登录 PC 后台后保持 WebSocket 在线，退出登录后断开连接。
- 后端通过 WebSocket 向 PC 后台推送新订单事件和顾客浏览事件。
- 用户订单支付成功并写入已支付状态后，后端向 PC 后台推送 `order_notify` 新订单事件。
- 支付成功后可向服务人员接单小程序推送待接订单提醒。
...
```
POST /api/v1/service-staff/login
```
...
```
POST /api/v1/service-staff/wechat-login
```
...
```
POST /api/v1/service-staff/register
```
...
**流程：** 注册申请提交后进入待审核状态，由商户管理员在 PC 后台审核，审核通过后服务人员方可登录接单小程序。
...
```
GET /api/v1/service-staff/orders/pending
...
```
POST /api/v1/service-staff/orders/{id}/accept
Authorization: Bearer {staff_token}
```
...
```json
{
  ...
  "data": {
    "id": 1,
    "accepted_by_staff_id": 1,
    "accepted_at": "2024-01-01T12:01:00Z"
  }
}
```
...
```
GET /api/v1/service-staff/orders/accepted
Authorization: Bearer {staff_token}
```
...
```
GET /api/v1/service-staff/orders/{id}
Authorization: Bearer {staff_token}
```
...
```
GET /api/v1/service-staff/statistics
Authorization: Bearer {staff_token}
```
...
#### 3.4.9 商家 WebSocket 连接（下单提醒 / 浏览提醒）
...
| 字段             | 类型     | 必填 | 说明                              |
| -------------- | ------ | -- | ------------------------------- |
| deduct\_amount | number | 否 | 押金扣除金额（损坏赔偿，0=全额退还；不得超过押金总额）    |
| remark         | string | 否 | 验机备注（写入 `rental_return_remark`） |

**业务规则：**
- 仅 `status=paid` 且 `total_deposit > 0` 且 `deposit_status=1`（待退还）的订单可发起归还。
- 调用后：
  - `rental_returned_at` = 当前时间
  - `deposit_deduct_amount` = `deduct_amount`
  - `deposit_refund_amount` = `total_deposit - deduct_amount`
  - `deposit_status` = `deduct_amount > 0 ? 3 : 2`
  - `deposit_refunded_at` = 当前时间
- 系统通过微信支付商户原路退还 `deposit_refund_amount` 给用户。
- 归还成功后，订单主状态仍保持 `paid`，需由商家后续手动核销为 `completed`。
...
```json
{
  ...
  "data": {
    ...
    "deposit_status": 2,
    "deposit_deduct_amount": 0.00,
    "deposit_refund_amount": 200.00,
    "deposit_refunded_at": "2026-08-12T10:00:00Z",
    "rental_returned_at": "2026-08-12T10:00:00Z",
    "rental_return_remark": "验机无损坏，全额退还"
  }
}
```
...
### 3.9 C端商城接口（扫码进入商户商城，前缀 /api/v1/store）

> **说明**：用户通过扫描商户二维码进入统一小程序商城，小程序携带商户ID参数，所有操作都在当前商户商城上下文中进行。
...
| 字段              | 类型   | 必填 | 说明                                            |
| --------------- | ---- | -- | --------------------------------------------- |
| product\_id     | int  | 是  | 商品ID                                          |
| spec\_info      | string | 否 | 规格信息                                          |
| quantity        | int  | 是  | 购买数量                                          |
| rental\_duration | int  | 否 | 租赁时长（按商品 `rental_unit` 计；**租赁商品必填**，一口价商品不传或传 0） |
...
> **租赁商品金额计算：**
> - 单行租金小计 = `rental_price × rental_duration × quantity`
> - 单行押金 = `deposit × quantity`
> - 订单 `total_amount` = 所有行租金小计之和（一口价商品仍按 `price × quantity` 累加）
> - 订单 `total_deposit` = 所有行押金之和（仅租赁订单 > 0）
> - 订单 `pay_amount` = `total_amount + total_deposit + delivery_fee - discount_amount`
...
> **说明**：支付参数中的 `paySign` 由系统级微信支付商户号签名，支付资金直接结算到商户的子商户账户。租赁订单的押金随租金一并支付，归还时由商家发起押金原路退还。
...
- **商家订单**：传入 merchant\_id 参数，获取用户在特定商家的订单（用于「我的」tab「我的订单」入口跳转）
...
| 字段                        | 所属表         | 描述                          |
| ------------------------- | ----------- | --------------------------- |
| `sub_mch_id`              | `merchants` | 已完成线下进件的商户子商户号，由商户在PC后台回填   |
| `payment_config_status`   | `merchants` | 支付配置状态：`0` 未完成、`1` 已完成      |

| 口径    | 内容                           |
| ----- | ---------------------------- |
| 线下进件  | 系统外部前置动作                     |
| 系统内表现 | 仅记录 `sub_mch_id`、支付配置状态 |
...
### 4.3 商户配送设置表 (merchant\_delivery\_settings)
...
| icon         | VARCHAR(512) | 分类图标URL（用于首页分类图标宫格） |
| sort         | INT         | 排序权重                |
...
| link\_type   | VARCHAR(32) | 跳转类型：product/category/none   |
| link\_value  | VARCHAR(128) | 跳转目标值（商品ID/分类ID）             |
| sort         | INT         | 排序权重                        |
| status       | TINYINT     | 状态：0禁用 1启用                  |
| created\_at  | DATETIME    | 创建时间                        |
| updated\_at  | DATETIME    | 更新时间                        |
...
| images                | JSON          | 图片数组                        |
| price                 | DECIMAL(10,2) | 售价（一口价商品的有效价格）              |
...
| sale\_type            | TINYINT       | 销售类型：1一口价 2租赁（默认 1）         |
| rental\_unit          | TINYINT       | 租赁计费周期：1按天 2按周 3按月（租赁商品必填）  |
| rental\_price         | DECIMAL(10,2) | 单位租金（租赁商品的有效租金，按一个计费周期计）    |
| deposit               | DECIMAL(10,2) | 押金（租赁商品收取，归还时退还）            |
| max\_rental\_duration | INT           | 最大租赁时长（0=不限；>0 时限制可选时长上限）   |
| deleted\_at           | DATETIME      | 删除时间                        |
| created\_at           | DATETIME      | 创建时间                        |
| updated\_at           | DATETIME      | 更新时间                        |

> **销售类型说明：**
> - `sale_type=1` 一口价：用户按 `price` 购买，下单、支付、配送/自提/堂食、核销完成，与原有流程一致。
> - `sale_type=2` 租赁：用户按 `rental_price × rental_duration + deposit` 支付（租金 + 押金），使用完成后由商家验机并退还押金（可扣减损坏费用）。下单时必须在 `items[].rental_duration` 中传入租赁时长。

### 4.9 商品规格表 (product\_specs)
...
| merchant\_id               | BIGINT        | 商家ID                              |
| total\_amount              | DECIMAL(10,2) | 商品总金额（租赁订单为租金合计，不含押金）             |
| total\_deposit             | DECIMAL(10,2) | 押金合计（仅租赁订单 > 0，下单时随租金一并支付）        |
| delivery\_fee              | DECIMAL(10,2) | 配送费                               |
| discount\_amount           | DECIMAL(10,2) | 优惠金额                              |
| pay\_amount                | DECIMAL(10,2) | 实付金额（= 商品金额 + 押金 + 配送费 - 优惠）      |
...
| accepted\_by\_staff\_id    | BIGINT        | 接单服务人员ID                          |
| accepted\_at               | DATETIME      | 接单时间                              |
| deposit\_status            | TINYINT       | 押金状态：0无押金 1待退还 2已退还 3已扣除（仅租赁订单有效） |
| deposit\_refund\_amount    | DECIMAL(10,2) | 押金退还金额                            |
| deposit\_deduct\_amount    | DECIMAL(10,2) | 押金扣除金额（验机后商家扣减的损坏赔偿）              |
| deposit\_refunded\_at      | DATETIME      | 押金退还时间                            |
| rental\_returned\_at       | DATETIME      | 租赁商品归还时间                          |
| rental\_return\_remark     | VARCHAR(256)  | 归还验机备注                            |
...
> **租赁订单补充说明：**
> - 租赁订单的 `paid` 状态在业务上等同于"租赁中"，需等待用户归还商品。
> - 归还由商家在 PC 后台订单详情触发，调用归还接口后：`rental_returned_at` 写入归还时间，`deposit_status` 置为 2（已退还）或 3（已扣除），`deposit_refund_amount` = `total_deposit - deposit_deduct_amount`，并原路退还押金。
> - 押金退还与订单主状态流转相互独立：租赁订单可能在 `paid` 状态下完成押金退还，随后由商家手动核销为 `completed`。
...
| price                 | DECIMAL(10,2) | 单价（一口价商品为售价；租赁商品为该行租金小计，便于订单汇总展示）     |
| quantity              | INT           | 数量                                    |
| spec\_info            | JSON          | 规格信息（如：大份/小份）                         |
| subtotal              | DECIMAL(10,2) | 小计（一口价=单价×数量；租赁=租金小计，不含押金）            |
| sale\_type            | TINYINT       | 销售类型：1一口价 2租赁（下单时快照）                  |
| rental\_unit          | TINYINT       | 租赁计费周期（下单时快照，仅租赁商品有值）                 |
| rental\_duration      | INT           | 租赁时长（按 rental\_unit 计，仅租赁商品有值）        |
| unit\_rental\_price   | DECIMAL(10,2) | 单位租金（下单时快照，仅租赁商品有值）                   |
| rental\_subtotal      | DECIMAL(10,2) | 租金小计 = unit\_rental\_price × rental\_duration × quantity |
| deposit               | DECIMAL(10,2) | 单件押金（下单时快照，仅租赁商品有值）                   |
| deposit\_deduct       | DECIMAL(10,2) | 单件押金扣除（归还时由商家填写，默认 0）                 |
| created\_at           | DATETIME      | 创建时间                                  |
...
### 4.19 用户行为事件表 (user\_behavior\_events)
...
| service\_staffs                | username                          | UNIQUE | 服务人员账号查询      |
| service\_staffs                | phone                             | BTREE  | 服务人员手机号查询    |
| orders                             | accepted\_by\_staff\_id           | BTREE  | 服务人员接单记录查询  |
...
| merchant:{id}          | HASH   | 5分钟  | 商户基本信息缓存    |
| merchant:{id}:products | LIST   | 5分钟  | 商户商品列表缓存    |
| dashboard:merchant     | STRING | 1分钟  | 商户数据看板缓存    |
| qrcode:{merchant\_id}  | STRING | 1小时  | 商户二维码缓存     |
...
| 页面                           | 功能描述       | 对接API                                       |
| ---------------------------- | ---------- | ------------------------------------------- |
| auth/login                   | 商户管理员登录（PC后台） | POST /api/v1/auth/admin/login            |
| merchant/settings            | 商户设置与员工能力  | GET/PUT /api/v1/merchant/settings           |
...
**第七批次：C端商城购物（扫码进入商城首页，tabBar 导航）**
...
| my/index          | 我的（用户信息+订单+地址+联系商户）           | GET /api/v1/user/orders                  |
...
#### 8.3.3 商家支付配置状态枚举 (merchants.payment\_config\_status)

| 值 | 状态标识       | 描述  | 说明  |
| - | ---------- | --- | --- |
| 0 | incomplete | 未完成配置 | 未回填 sub\_mch_id |
| 1 | complete | 已完成配置 | 已回填 sub\_mch\_id，可正常拉起支付 |

#### 8.3.4 商品状态枚举 (products.status)
...
#### 8.3.5 商品分类状态枚举 (categories.status)
...
#### 8.3.6 商户员工状态枚举 (merchant\_staffs.status)
...
#### 8.3.8 系统公告状态枚举 (announcements.status)
...
#### 8.3.9 云打印机状态枚举 (cloud\_printers.status)
...
#### 8.3.10 打印记录状态枚举 (print\_logs.status)
...
#### 8.3.11 退款记录状态枚举 (refunds.status)
...
#### 8.3.12 配送类型枚举 (orders.delivery\_type)
...
#### 8.3.13 服务人员状态枚举 (service\_staffs.status)
...
1. **商户 PC 后台**：通过 `web-admin/` 登录后进入商户管理后台，用于商品管理、订单核销、支付配置、服务人员管理与数据分析。
2. **服务人员接单小程序**：服务人员登录后进入接单小程序，用于待接订单查看、接单操作与接单统计。
3. **C 端商城小程序**：用户浏览商城并下单的链路测试，由于开发阶段无法总是通过扫码直达指定商户商城，因此提供测试入口页与编译模式两种方式。
...
| 角色       | 账号         | 密码            | 说明                 |
| -------- | ---------- | ------------- | ------------------ |
| 商户管理员   | `merchant` | `merchant123` | 用于商户PC后台登录与功能验证 |
| 服务人员   | `staff`    | `staff123`    | 用于服务人员接单小程序登录与接单联调 |
...
| 商品管理   | 商品列表、商品详情、分类管理           | `/products` / `/products/:id` / `/categories`         |
| 订单管理   | 订单列表、订单详情、订单核销           | `/orders` / `/orders/:id`                             |
| 服务人员管理 | 服务人员列表、新增、审核注册申请         | `/service-staff`                                      |
| 数据分析   | 商户统计                     | `/analytics`                                          |
| 设置     | 商户资料、支付配置、修改密码   | `/settings`                                           |
...
- 入口：服务人员接单小程序
- 使用测试账号：`staff / staff123`

#### 使用步骤

1. 在微信开发者工具中打开服务人员接单小程序项目
2. 使用测试账号 `staff / staff123` 登录
3. 登录成功后进入待接订单列表
4. 按需继续验证接单、已接订单、订单详情与接单统计

#### 可测试功能

| 模块     | 功能点            | 说明                                           |
| ------ | -------------- | -------------------------------------------- |
| 待接订单   | 查看待接订单列表       | 支持空数组兜底，展示订单基本信息与配送方式                       |
| 接单     | 接受订单           | 接单为原子操作，接单成功后订单写入 `accepted_by_staff_id`    |
| 已接订单   | 查看已接订单列表       | 展示已接订单与接单时间                                  |
| 订单详情   | 查看订单详情         | 展示订单商品明细、配送信息、支付信息                          |
| 接单统计   | 查看今日与累计接单统计    | 展示接单数量与接单金额                                  |
| 注册申请   | 服务人员注册申请       | 提交后进入待审核状态，由商户管理员在PC后台审核                     |
...
4. 或直接点击商家列表中的商家
5. 点击"进入商家店铺"按钮
6. 进入商城首页后可进行商品浏览、加入购物车、下单支付等操作
...
#### 9.5.2 测试方法二：PC后台 WebSocket 联调页

##### 功能说明

提供专门的 `pages/store/order-sound-test.vue` 页面，用于验证商户PC后台登录成功后是否已建立 WebSocket 连接。
...
> 增量开发按"期"推进，每期交付可独立编译运行。状态：✅ 已完成 / ⏳ 进行中 / ⬜ 待启动

### 10.1 第 0 期：系统清理与脚手架

- ✅ 删除 8 张外卖零售相关历史表，清理/补齐 17 个字段，保留自营商家数据（merchants.id=1）
- ✅ 新建 `staff-miniprogram/` 服务人员端（uni-app + TS + Vite），含 4 个 tab 页脚手架

### 10.2 第 1 期：商品侧一口价/租赁表单与康养套餐

- ✅ 定义 5 类业务商品（1=辅具零售、2=辅具租赁、3=康养套餐、4=陪诊服务、5=科普资讯），自动绑定销售模式（一口价/租赁）
- ✅ 后端扩展 `Product.ProductType`、`normalizeProductType()`，支持 `service_content` JSON 序列化
- ✅ PC 后台商品类型 Tab、编辑对话框按类型自动联动销售模式与租赁字段，新增康养套餐编辑器

### 10.3 第 2 期：C 端商城按商品类型差异化展示与下单

- ✅ C 端类型扩展与 `normalizeServiceContent()`
- ✅ 首页 5 类分区、统一颜色标签；商品页差异化渲染（套餐周期/适用人群、陪诊/资讯描述区）
- ✅ 购物车与结算页透传 `product_type`，后端按类型区分订单类型

### 10.4 第 3 期：服务人员端工单全链路打通

- ✅ 新增 `service_staffs` 表与 JWT 认证中间件，服务人员认证（注册/登录/微信登录）
- ✅ 工单管理接口（待接/已接/详情/接单/签到/签退）与统计接口
...
- ✅ **登录与布局**：`LoginView` 改为商家登录；`AppLayout` 单商户菜单；`router` 增加全局前置守卫
- ✅ **页面重写**：`DashboardView`（单商户工作台+快捷入口）、`OrderListView`（去商家筛选、新增订单类型/工单状态筛选）、`OrderDetailView`（工单字段+核销入口）、`MerchantStatsView`（单商户数据分析，支持周期切换/趋势/商品排行 Top10）、`MerchantProfileView`（基础信息+改密）
